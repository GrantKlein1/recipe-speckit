...
4. In Cursor’s terminal (from the `backend` folder), start the API by entering:
  npm start
   Server listens on [http://localhost:3200](http://localhost:3200) (routes under `/recipeapi`). Keep this Cursor terminal panel open while the API is running.
...
4. Open [http://localhost:8082](http://localhost:8082) in a browser (this SpecKit layout uses port **8082**; the original standalone frontend README mentioned 8081).
...
- Backend: in a browser, open [http://localhost:3200/](http://localhost:3200/) — you should see a welcome message for the recipe backend.
- Frontend: login / published recipes UI loads at [http://localhost:8082](http://localhost:8082).
...
| 4   | [features/feature-4-recipe-ingredient-management](features/feature-4-recipe-ingredients-management.md) | `feature/4-recipe-ingredient-management`  | 1—2—3 |
